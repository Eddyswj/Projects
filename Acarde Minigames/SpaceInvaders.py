...
class Player:
    def __init__(self, canvas, master, x, y, name, lives):  # Where x and y are the coordinates of the object.
        ...
        self.lives = lives
        self.master = master
        self.label = canvas.create_text(x, y - 30, text=f"{name} Lives: {self.lives}")
        self.canvas = canvas
        self.player = canvas.create_oval(x - 10, y - 10, x + 10, y + 10, fill='red')
    ...
    def decrease_lives(self):
        self.lives -= 1
        if self.lives >= 0:
            self.canvas.itemconfig(self.label, text=f"Player1 Lives: {self.lives}")  # To modify the parameters of the tkinter object.
        if self.lives == 0:
            self.show_game_over()
            print("Game over")

    def show_game_over(self):
        game_over_label = tk.Label(self.canvas, text="Game Over", font=("Helvetica", 24), fg="red")
    ...
    def generate_enemy(self, canvas_width, canvas_height):  # Takes in the height and width of canvas to ensure enemies spawn
        while True:
            ...
            if not overlap:  # If overlap is False, generate new enemies.
                color = random.choice(['red', 'blue', 'green', 'yellow'])  # Probably can do something with the colors. For now, the different colors mean nothing.
    ...
    def move(self, red_line, lives, timer):
        for enemy in self.enemies:
            if timer > 30:
                ...
                self.canvas.move(enemy, 0, 1)  # Probably should add more features. If the players survive for a certain period of time, can probably increase the speed of the enemies.
...
class Projectile:
    def __init__(self, canvas, x, y):
        self.canvas = canvas
        self.projectile = canvas.create_oval(x + 5, y + 5, x, y, fill='red')
    ...
    def move(self, enemies):
        ...
        if coords:
            if coords[1] < 0:
                ...
                self.check_collision(enemies)  # Check for collision with the enemies.
...
class Game(tk.Frame):
    def __init__(self, master, player_name):
        ...
        self.width = 600
        self.height = 600
        self.canvas = tk.Canvas(self, width=self.width, height=self.height)  # Creates the canvas
        self.canvas.pack()
        self.pack()
    ...
    def create_projectile(self):
        ...
        projectile = Projectile(self.canvas, center_x, center_y)
        self.projectiles.append(projectile)  # Adds the projectiles to the list.
        self.after(500, self.create_projectile)  # Calls this function every 0.5 seconds. Basically spawn every projectile every 0.5s.
    ...
    def player_wins(self):
        ...
        print("Congratulations! You survived for 45 seconds!")
    ...
    def generate_new_enemies(self):
        ...
        self.after(2000, self.generate_new_enemies)  # Schedule the next call after 5 seconds
