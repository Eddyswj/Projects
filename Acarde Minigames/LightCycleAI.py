...
def in_boundary(head, res):
    ...
    """
    min_x_bound, min_y_bound = -res/2 + res*0.02, -res/2 + res*0.04
    max_x_bound, max_y_bound = res/2 - res*0.04, res/2 - res*0.02
...
def draw():
    ...
    # possible_collisions = {}
    # for possible_dir in p2_dir.routes:
    #     p2_pos_copy = p2_pos.copy()
    #     p2_testroutes = []
    #     for _ in range(100):
    #         p2_pos_copy.translate(vector(possible_dir[0], possible_dir[1]))
    #         p2_testroutes.append(p2_pos_copy)
    #     possible_collisions.update({possible_dir[2]:[i for i in p2_testroutes if i in p1_trail]})
    possible_collisions = []
    p2_pos_copy = p2_pos.copy()
    p2_testroutes = []
    for _ in range(100):
        p2_pos_copy.translate(vector(p2_dir.x, p2_dir.y))
        p2_testroutes.append(p2_pos_copy.copy())
    possible_collisions = [i for i in p2_testroutes if i in p1_trail]
    if len(possible_collisions) > 0 or (-res/2 + res*0.1 > p2_pos.x or p2_pos.x > res/2 - res*0.1) or (-res/2 + res*0.1 > p2_pos.y or p2_pos.y > res/2 - res*0.1):
        p2_dir.rotate(p2_dir.changes[random.randint(0, 1)])
...
def main():
    ...
    """
    global s, t, res, diff, p1_pos, p2_pos, p1_dir, p2_dir, p1_trail, p2_trail
    ...
    # sys.stdout.write("Game starts in 5 seconds")
    # backspace = "\b"*9
    ...
    s.onkey(lambda: p2_dir.rotate("up"), 'o')
    s.onkey(lambda: p2_dir.rotate("left"), 'k')
    s.onkey(lambda: p2_dir.rotate("down"), 'l')
    s.onkey(lambda: p2_dir.rotate("right"), ';')
